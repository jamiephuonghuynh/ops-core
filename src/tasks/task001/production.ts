...
import { finalizeInputProcessing, failInputProcessing } from "../../db/input-processing";
import { getActiveBinding, listActiveBindings } from "../../db/resource-bindings";
import { setExecutionRuntimeConfig } from "../../db/executions";
...
async function uploadEvidence(env: Env, executionId: string, value: unknown) {
  const bytes = new TextEncoder().encode(JSON.stringify(value)).buffer as ArrayBuffer;
  const hash = await sha256HexBytes(bytes);
  const result = await uploadR2Artifact(env, { executionId, artifactRole: "TASK001_SAFE_WRITE_REPORT", direction: "EVIDENCE", fileName: "task001-safe-write-report.json", mimeType: "application/json", idempotencyKey: `TASK001:P5:REPORT:${hash}`, suppliedSha256: hash, bytes, requestId: `WF_${executionId}` });
  if (result.kind === "ERROR") throw new Error(`${result.error}:${result.message}`);
...
async function snapshotGapp(env: Env, executionId: string, sourceResourceId: string) {
  ...
  if (!resource?.external_id || resource.resource_type !== "DRIVE_FILE" || resource.provider !== "GOOGLE") throw new Error("TASK001_GAPP_FILE_RESOURCE_INVALID");
  ...
  const parsed = parseFirstWorksheetXlsx(await response.arrayBuffer());
  const dataset = { headers: parsed.headers, rows: parsed.rows };
  const canonical = JSON.stringify(dataset);
  return snapshotJson(env, executionId, "TASK001_INPUT_GAPP_NORMALIZED", `task001-gapp-${sourceResourceId}.snapshot.json`, dataset, `TASK001:P5:GAPP:${sourceResourceId}:${await sha256HexBytes(new TextEncoder().encode(canonical).buffer as ArrayBuffer)}`);
}
...
export async function runTask001ProductionFoundationWorkflow(env: Env, step: WorkflowStep, execution: ExecutionRow, task: TaskDefinitionRow, recordedStep: RecordedStep) {
  if (task.definition_version !== TASK001_DEFINITION_VERSION || task.execution_mode !== "SAFE_WRITE_TEST") throw new Error(`TASK001_UNSUPPORTED_DEFINITION:${task.definition_version}/${task.execution_mode}`);
  const payload = parsePayload(execution);
  if (payload.executionMode !== "TEST_SAFE_WRITE") throw new Error("TASK001_PHASE5_REQUIRES_TEST_SAFE_WRITE");
  ...
  const config = await recordedStep("T001_01_RESOLVE_CONFIG", 1, {}, async () => {
    ...
    const bindings = await applyOverrides(env, bindingMap(activeBindings), payload);
    ...
    return { mappingSetId: mappingSet.mapping_set_id, configVersion: mappingSet.mapping_version, bindings: Object.fromEntries([...bindings].map(([role, b]) => [role, b])), mappings: entries };
  });
  ...
  const input = await recordedStep("T001_02_RESOLVE_INPUT", 2, { bindingRole: "GAPP_EXPORT" }, async () => resolveLatestEligibleTask001Input(env, { executionId: execution.execution_id, binding: bindings.get("GAPP_EXPORT")! }));
  if (!input.found) return { status: "WARNING" as const, resultCode: "NO_NEW_INPUT", resultMessage: "No eligible NEW XLSX input was found", reportResourceId: null };
  ...
  try {
    ...
    if (compute.resultCode === "NO_VENDOR_ROWS") {
      const finalized = await recordedStep("T001_08_FINALIZE_INPUT_STATE", 8, { inputProcessingId: input.inputProcessingId }, async () => {
    ...
    const canonicalCommit = await recordedStep("T001_06_COMMIT_GAPP_ORDER", 6, { newCount: canonicalClaims.newRows.length, identicalCount: canonicalClaims.identicalRows.length }, async (attempt) => {
      ...
      if (result.kind === "ERROR") {
        if (result.error === "OUTPUT_COMMIT_UNKNOWN") {
          await markBusinessClaimsUnknown(env, canonicalClaims.newClaimIds);
      ...
      return { committedRows: result.appendedRows, identicalRows: canonicalClaims.identicalRows.length, idempotentReplay: result.idempotentReplay, unknown: false };
    });

    if (canonicalCommit.unknown) throw new Error("OUTPUT_COMMIT_UNKNOWN:Canonical GAPP_ORDER append result is ambiguous; automatic retry is blocked");
    ...
    const deliveryCommit = await recordedStep("T001_07_COMMIT_SMARTLINK_DELIVERY", 7, { newCount: deliveryClaims.newRows.length, identicalCount: deliveryClaims.identicalRows.length }, async (attempt) => {
      ...
      try { await appendExecutionEvent(env, execution.execution_id, "TASK001_DELIVERY_COMMITTED", { deliveredOrderCount: result.appendedRows, deliveryResourceId, deliveryBusinessUri: (await getResource(env, deliveryResourceId))?.business_uri ?? null }); } catch {}
      return { committedRows: result.appendedRows, identicalRows: deliveryClaims.identicalRows.length, idempotentReplay: result.idempotentReplay, unknown: false };
    });

    if (deliveryCommit.unknown) throw new Error("OUTPUT_COMMIT_UNKNOWN:Smartlink delivery append result is ambiguous; automatic retry is blocked");
    ...
    const resultCode = warning ? "TASK001_SAFE_WRITE_WARNING" : "TASK001_SAFE_WRITE_SUCCESS";
    const finalized = await recordedStep("T001_08_FINALIZE_INPUT_STATE", 8, { inputProcessingId: input.inputProcessingId }, async () => {
    ...
    return {
      status: warning ? "WARNING" as const : "SUCCESS" as const,
      resultCode,
      resultMessage: `Canonical committed ${canonicalCommit.committedRows}; Smartlink delivery committed ${deliveryCommit.committedRows}; ${compute.skipped.length} non-Smartlink rows skipped.`,
