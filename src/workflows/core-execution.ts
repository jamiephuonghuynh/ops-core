...
export class CoreExecutionWorkflow extends WorkflowEntrypoint<Env, CoreExecutionWorkflowParams> {
  async run(event: WorkflowEvent<CoreExecutionWorkflowParams>, step: WorkflowStep): Promise<unknown> {
    ...
    try {
      ...
      const prepare = await recordedStep(this.env, step, executionId, "STEP_01_PREPARE", 1, { taskId: execution.task_id, taskVersion: execution.task_version }, async () => ({ prepared: true, config }));
      const processed = await recordedStep(this.env, step, executionId, "STEP_02_PROCESS", 2, { prepared: prepare.prepared }, async (attempt) => {
        ...
        if (failStepCode === "STEP_02_PROCESS" && attempt <= failAttempts) throw new Error(`FOUNDATION_TEST_INTENTIONAL_FAILURE attempt=${attempt}`);
